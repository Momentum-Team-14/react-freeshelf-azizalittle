...
export const App = () => {
  ...
  return (
    <div className='container'>
      <h1 class="title is-1">FreeShelf</h1>
...
const Book = ({ title, author, url, shortDescription, coverImageUrl, publisher, publicationDate, detailedDescription }) => {
  ...
  return (
    <div style={{ border: '1px solid green', margin: '10px' }}>
      ...
      <figure class="image is-3by2"><img src = {imageBroken ? 'https://cdn-icons-png.flaticon.com/512/107/107817.png' : coverImageUrl} onError={() => setImageBroken(true)} alt={"Cover for " + title}></img></figure>

      { title ? <h4 class="title is-4">{title}</h4> : "" }
      { author ? <h5 class="subtitle is-5">{author}</h5> : "" }
      { publisher ? <p>{shortDescription}</p> : ""}
      <button onClick={() => handleClick()}>
        {expanded ? 'Less' : 'More'} info
      </button>
        {expanded ? ( url ? <a class="book-url" href="{url}">{url}</a> : '') : ''}
        {expanded ? ( publisher ? <p>Publisher: {publisher}</p> : '') : ''}
        {expanded ? ( publicationDate ? <p>Publication Date: {publicationDate}</p> : '') : ''}
        {expanded ? ( detailedDescription ? <p>Detailed Description: {detailedDescription}</p> : '') : ''}
